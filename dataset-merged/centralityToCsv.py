...
dsFolderName = 'asap7V2'
MIXEDTEST     = False
dsFolderName2 = 'nangateV2'
...
class DataSetFromYosys( DGLDataset ):
    ...
    def export_graph_data_to_csv(self, graph, designPath):
        if feat2d in graph.ndata:
            feature_data = graph.ndata[feat2d]
            if isinstance(feature_data, torch.Tensor):
                feature_data = feature_data.numpy()

            df_features = pd.DataFrame(feature_data, columns=self.namesOfFeatures)
            dfs_to_combine = [df_features]

            for attr_name in graph.ndata.keys():
                if attr_name != feat2d:  # Avoid adding feat2d again
                    attr_data = graph.ndata[attr_name]
                    if isinstance(attr_data, torch.Tensor):
                        attr_data = attr_data.numpy()

                    if attr_data.ndim > 1:
                        for i in range(attr_data.shape[1]):
                            dfs_to_combine.append(pd.DataFrame(attr_data[:, i], columns=[f"{attr_name}_{i}"]))
                    else:
                        dfs_to_combine.append(pd.DataFrame(attr_data, columns=[attr_name]))

            combined_df = pd.concat(dfs_to_combine, axis=1)
            if 'id' in combined_df.columns:
                combined_df.set_index('id', inplace=True)

            combined_df.to_csv(f"{designPath}/{graph.name}-data.csv", index=True)
        else:
            print("No 'feat2d' data available in the graph.")
...
#QQ PLOT WORKING
    ...
    def process( self ):
        for path in self.graphPaths:
                ...
                self.export_graph_data_to_csv(graph, path)
    ...
    def _process_single( self, designPath ):
        ...
        print( "###########################################\n" )
        # if 'black' not in str( designPath ).rsplit( '/' )[-1]:
        #     return
...
if __name__ == '__main__':
    ...
    if not MANUALABLATION:
        ...
        ablationList = [ ( 'inDegree', 'outDegree', 'eigen', 'pageRank' ) ] #('outDegree',), ('inDegree',), ('input_pins',), ('output_pins',), ('inDegree','outDegree'), ('input_pins','output_pins') ]
